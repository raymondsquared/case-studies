...
export function validateConfig(config: Config): void {
  ...
  if (typeof config.terraformOrganisation !== 'string' || config.terraformOrganisation.trim() === '') {
    throw new Error('Config validation error: terraformOrganisation is required and cannot be empty.');
  }
  ...
  if (typeof config.enableSecretsManager !== 'boolean') {
    throw new Error('Config validation error: enableSecretsManager is required and must be a boolean.');
  }
  if (typeof config.tags !== 'object' || config.tags === null || Array.isArray(config.tags)) {
    throw new Error('Config validation error: tags is required and must be an object with string values.');
  }
}
