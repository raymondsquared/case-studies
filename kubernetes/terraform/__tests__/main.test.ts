...
describe('main', () => {
  ...
  describe('Stack Creation and Synthesis', () => {
    ...
    testCases.forEach(
      ({ name, environment, region, vendor, stackClass, publicVPC, expectedConfig }) => {
  ...
  describe('RemoteBackend Configuration', () => {
    ...
    it('Given custom terraform hostname from environment, When creating RemoteBackend, Then it should US_EAST custom hostname', () => {
      const environment = Environment.DEVELOPMENT;
      const region = Region.AUSTRALIA_EAST;
      const vendor = Vendor.AWS;
      const terraformOrganisation = 'test-org';
...
describe('Vendor-specific environment', () => {
  it('Given AWS vendor and missing AWS_ACCOUNT_ID, When running main, Then it should exit with error and log the correct message', () => {
    process.env.ENVIRONMENT = Environment.DEVELOPMENT;
    process.env.REGION = Region.AUSTRALIA_EAST;
    process.env.VENDOR = Vendor.AWS;
    process.env.TERRAFORM_ORGANISATION = 'test-org';
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      'Failed to synthesize Terraform configuration:',
      expect.objectContaining({ message: 'AWS_ACCOUNT_ID environment variable is required but not set.' })
    );
