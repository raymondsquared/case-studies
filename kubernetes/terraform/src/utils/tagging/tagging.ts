...
export class TaggingUtility {
  ...
  getTags(inputTags: Tags = {}): Tags {
    ...
    let name = tags['name'] || this.config.name;
    if (tags['nameSuffix']) {
      name = `${name}-${tags['nameSuffix']}`;
      tags['name'] = name;
    }

    const _env = tags['environment'];
    const inputResourceType = inputTags['resourceType'];
    const resourceType =
      inputResourceType !== undefined ? inputResourceType : this.config.resourceType;

    // Actual AWS tagging for Name
    const resourceName = `${cleanString(name)}-${cleanEnvironment(_env as Environment)}-${cleanString(resourceType)}-${cleanString(this.config.region)}`;
  ...
  private validateTags(inputTags: Tags): void {
    const inputName = inputTags['name'];
    const name = inputName !== undefined ? inputName : this.config.name;
    if (!name || name.trim() === '') {
